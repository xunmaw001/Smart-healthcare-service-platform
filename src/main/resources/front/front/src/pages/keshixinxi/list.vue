...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>科室号</div>
        <el-input v-model="formSearch.keshihao" placeholder="科室号" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>科室名称</div>
        <el-input v-model="formSearch.keshimingcheng" placeholder="科室名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	  ...
	  <el-button v-if="isAuth('keshixinxi','新增')" :style='{"cursor":"pointer","border":"1px solid #db961f","padding":"0px 15px","boxShadow":"1px 1px 3px #f8a412","margin":"0px 10px 0 0","color":"#fff","outline":"none","borderRadius":"4px","background":"#f8a412","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/keshixinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","margin":"0 0 10px","background":"none","order":"3"}'>
		...
		<div class="list1 index-pv1" :style='{"border":"1px solid #dfdfdf","padding":"20px 10px","margin":"16px 0 0 0","overflow":"hidden","borderRadius":"16px","flexWrap":"wrap","background":"#fff","display":"block","width":"1200px","height":"auto"}'>
			<div :style='{"margin":"0 1% 20px 1%","borderRadius":"8px","background":"none","display":"inline-block","width":"23%","position":"relative","float":"left","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"padding":"0 10px","boxShadow":"1px 1px 1px #ddd","margin":"4px 0 0 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"8px","background":"radial-gradient(circle, rgba(250,250,250,1) 0%, rgba(220,230,250,0.5) 100%)","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.keshimingcheng}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
        this.$http.get('option/keshifenlei/keshifenlei').then(res => {
          if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.keshimingcheng = '%' + this.indexQueryCondition + '%';
          this.indexQueryCondition = '';
        }
        if (this.formSearch.keshihao != '') searchWhere.keshihao = '%' + this.formSearch.keshihao + '%';
        ...
        this.$http.get('keshixinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/keshixinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
