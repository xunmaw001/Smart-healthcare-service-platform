    ...
    >
          <el-form-item :style='{"border":"1px solid #dfdfdf","padding":"10px","boxShadow":"1px 2px 3px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(230,230,230,1) 100%)"}' label="药品类型" prop="yaopinleixing">
            <el-input v-model="ruleForm.yaopinleixing" 
                placeholder="药品类型" clearable ></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('yaopinleixing/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('yaopinleixing/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
